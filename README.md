...
where `P_d` and `G_d` are day `d`'s mean price and mean output, `cov_d` is the within-day price/output covariance, and `D = sum(output) × mean(price)`. The seasonal term answers *is the output in the expensive months?* The intraday term answers *is it in the expensive hours?*

The intraday term measures **within-day price–output alignment**. That is a description of the arithmetic, not a claim about cause. Cannibalisation is one interpretation of it — the one that applies once enough PV is on the system to depress the hours it generates in — and this market is not there yet, which is why the term is currently *positive* in five of seven years. Naming the quantity after its high-penetration interpretation would assume the conclusion.

`tests/test_capture_decomposition.py` asserts the identity holds, that a purely seasonal price signal puts nothing in the intraday term, and that an artificial midday trough puts nothing in the seasonal term.
...
> The horizontal-plane error was worth **10.7 percentage points** on the worst year (71.9% → 82.5%) and 3.7 points on the period mean. It moved the headline in the direction that flattered the "solar is badly discounted" story, which is exactly the direction an error is easiest not to notice.
